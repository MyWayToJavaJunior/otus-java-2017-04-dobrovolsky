...
public class ReflectionHelper {
    ...
    public static <T extends DataSet> List<String> getColumnsAndValuesString1(Class<T> clazz) {
        StringBuilder ret = new StringBuilder();
        Field[] fields = clazz.getDeclaredFields();
        ...
        ret = new StringBuilder(annotatedFields.get(annotatedFields.size() - 1));
        ret.delete(ret.length() - 2, ret.length()).append(")");
        annotatedFields.remove(annotatedFields.size() - 1);
        annotatedFields.add(ret.toString());
